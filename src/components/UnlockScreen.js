...
export class UnlockScreen {
  ...
  render() {
    ...
    this.container.className = 'fixed inset-0 z-[9999] flex items-center justify-center overflow-y-auto py-6 bg-white/90 dark:bg-zinc-950/90 backdrop-blur-xl transition-all duration-500 font-sans';
    ...
    this.container.innerHTML = `
      <div id="unlock-card" class="bg-white dark:bg-zinc-900/60 border border-zinc-200/80 dark:border-zinc-800/80 p-7 rounded-2xl shadow-2xl text-center max-w-sm w-full mx-4 my-auto flex flex-col gap-4 transition-all duration-300 transform scale-95 opacity-0">
        <!-- Shield Lock Icon -->
        <div class="w-14 h-14 bg-zinc-100 dark:bg-zinc-800/40 text-zinc-900 dark:text-white rounded-2xl flex items-center justify-center mx-auto border border-zinc-200 dark:border-zinc-700/30">
        ...
        <div class="space-y-1.5">
          <h1 class="text-lg font-black tracking-tight text-zinc-950 dark:text-white uppercase">${title} <span class="text-[10px] bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400 px-1.5 py-0.5 rounded font-mono font-normal tracking-normal lowercase border border-zinc-200 dark:border-zinc-750">beta</span></h1>
          <h2 class="text-xs font-bold text-zinc-700 dark:text-zinc-350">${subtitle}</h2>
          <p class="text-[10px] text-zinc-500 dark:text-zinc-500 leading-normal px-2">${desc}</p>
        </div>
        ...
        <form id="unlock-form" class="space-y-3">
          <div class="relative">
            <input
              ...
              class="w-full px-3.5 py-2.5 bg-zinc-50 dark:bg-zinc-950/70 border border-zinc-200 dark:border-zinc-800 text-xs rounded-xl text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-zinc-400 dark:focus:ring-zinc-650 transition-all font-mono text-center"
            />
          ...
          <button
            ...
            class="w-full py-2.5 bg-zinc-900 text-white hover:bg-zinc-700 dark:bg-white dark:text-black dark:hover:bg-zinc-200 rounded-xl text-xs font-bold transition-all shadow-md cursor-pointer flex items-center justify-center gap-1.5"
          >
        ...
        <!-- Language toggle -->
        <div class="pt-2 border-t border-zinc-200/60 dark:border-zinc-800/40 flex justify-center gap-4 text-[9px] font-sans text-zinc-500">
          <button id="unlock-lang-ru" class="hover:text-zinc-700 dark:hover:text-zinc-300 cursor-pointer ${isRu ? 'text-zinc-900 dark:text-zinc-300 font-bold' : ''}">RU</button>
          <button id="unlock-lang-en" class="hover:text-zinc-700 dark:hover:text-zinc-300 cursor-pointer ${!isRu ? 'text-zinc-900 dark:text-zinc-300 font-bold' : ''}">EN</button>
        </div>
      </div>
    `;
  ...
  bindEvents(errorEmpty, errorIncorrect) {
    ...
    if (form) {
      form.onsubmit = async (e) => {
        ...
        submitBtn.innerHTML = `<span class="w-3.5 h-3.5 border-2 border-zinc-400 border-t-white dark:border-zinc-600 dark:border-t-black rounded-full animate-spin"></span>`;
