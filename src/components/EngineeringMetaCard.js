...
export class EngineeringMetaCard {
   ...
   * @param {string} params.applicabilityScope - Scope of verification
   * @param {string} params.limitationNotes - Operational limits and notes
   * @param {string} lang - Active language ('en' | 'ru')
   * @returns {string} HTML string
   */
  static render({ source, revisionDate, lastUpdated, confidenceLevel, applicabilityScope, limitationNotes }, lang) {
      ...
      <div class="space-y-3 font-sans text-xs">
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-zinc-655 dark:text-zinc-350 text-[10.5px]">
          <div>
            ...
            <span class="font-mono">${revisionDate || getPlaceholder('validation', lang)}</span>
          </div>
          <div>
            ...
            <span class="font-medium truncate block max-w-[120px]" title="${source || ''}">${source || getPlaceholder('no_data', lang)}</span>
          </div>
          <div>
            <span class="block text-[8.5px] font-bold text-zinc-400 dark:text-zinc-550 uppercase tracking-widest mb-0.5">${t('confidence_title')}</span>
          ...
          <div>
            ...
            <span class="font-mono">${lastUpdated || getPlaceholder('validation', lang)}</span>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2.5 border-t border-zinc-100 dark:border-zinc-800/80 text-zinc-655 dark:text-zinc-350 text-[10.5px]">
          <div>
            <span class="block text-[8.5px] font-bold text-zinc-400 dark:text-zinc-555 uppercase tracking-widest mb-0.5">${t('columns.applicability_scope')}</span>
            <span class="font-medium">${translateDbText(applicabilityScope || '', lang) || getPlaceholder('no_data', lang)}</span>
          </div>
          <div>
            <span class="block text-[8.5px] font-bold text-zinc-400 dark:text-zinc-550 uppercase tracking-widest mb-0.5">${t('columns.limitation_notes')}</span>
            <span class="font-medium text-amber-600 dark:text-amber-450">${translateDbText(limitationNotes || '', lang) || getPlaceholder('na', lang)}</span>
          </div>
        </div>
      </div>
    `;
